...
xdg-types - show the content types provided by a desktop application

=head1 SYNOPSIS

B<xdg-types> [I<options>] [--] I<APPID> [...]

=head1 DESCRIPTION

B<xdg-types> takes one or more arguments.  For each of its arguments, it
...
The options that are detailed below are recognized by B<xdg-types>.
B<xdg-types> uses L<getopt_long_only(3)>, so unambiguous abbreviations
...
Output list of effective lookup paths that B<xdg-types> is using.
...
=item B<-m>, B<--mimetypes>

For each desktop entry, also print to standard output an indented
list of the mime types supported by the desktop entry, one mime type per
line.
...
=head1 EXAMPLES
 ...
 $> xdg-types firefox
 /usr/share/xde/applications/firefox.desktop
         text/html
         text/xml
...
The following environment variables are examined by B<xdg-types>:

=over

=item B<HOME>

Used by B<xdg-types> to determine the user's home directory.

=item B<XDG_DATA_HOME>

Used by B<xdg-types> to determine the user's XDG data directory.  This
...
Used by B<xdg-types> to determine the system XDG data directories.
...
Used by B<xdg-types> to determin the user's XDG configuration directory.
...
Used by B<xdg-types> to determine the system XDG configuration
