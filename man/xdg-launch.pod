...
Only one of B<--xsession> or B<--autostart> can be specified.
...
considering the application as launched.  The default is fifteen
seconds.  This is the same default as L<xtoolwait(1)>.
...
application or command.  The default behavior is to not wait for any
desktop resource before launching.
