...
Specify the filename of the recent history file.  When in exec mode,
...
command.  When no session option is specified, B<xdg-launch> will not
await any desktop resource before launching.
