...
specify the autostart phase (.e.g. C<AutostartPhase>).  When the
B<--autostart> option is option is specified, the setting of these key
fields may affect the default settings of the session options, above.
