...
B<xdg-launch> [I<OPTIONS>] [I<APPSPEC>] [I<FILE>|I<URL>]
...
information about the final desktop file selected to launch the the
actions that would have been taken to launch the desktop entry if this
option had not been specified.  This is equivalent to a dry run.
...
Only one of B<--xsession>, B<--autostart> or B<--session> can be specified.
...
differences, the launcher operates in the same fashion.  This option is
intended for launching autostart entries.
...
When no X session is specified, waits after the autostart procedure
...
launched application should be reparented if its parent terminates.
This option defaults to the environment variable B<${XDG_SESSION_PID}>.
To keep it from being set automatically, set it to zero.  This is useful
for applications launched by a window manager to ensure that if the
window manager crashes, it may be restarted by the session and the
application child processes will be re-parented to the session.
...
=head1 JOB CONTROL OPTIONS

The following job control options provide an equivalent functionality to
...
default when the window manager supports startup notification is to not
provide any assistance.  This option requests that B<xdg-launch> provide
startup notification assistance to the window manager.

=item B<--no-assist>

Normally B<xdg-launch> will determine whether the window manager and
application toolkit requires assistance with startup notification.  The
default when the window manager does not support startup notification is
to provide assistance.  This option requests that B<xdg-launch> not
provide startup notification assistance to the window manager.

=item B<-T>, B<--toolwait>

Normally, B<xdg-launch> will not exit, but will replace the foreground
process with the application using L<execve(1)>.  When this option is
...
process.  It attempts to become an session leader and a process group
leader before launching children.  This should ultimately leave any
child processes as children of the window manager.

When no I<APPID> or B<--exec> command is specified, instead of starting
a window manager, waits until all child processes have exited and then
exits itself.
...
B<xdg-launch> has no known bugs.
