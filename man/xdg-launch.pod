...
B< xdg-launch> [I<OPTIONS>] [I<APPSPEC> [I<FILE>|I<URL>]...|I<COMMAND>]
 B<xdg-launch> {B<-U>|B<--autostart>} [I<OPTIONS>] [I<APPID>|I<DESKTOP>|I<COMMAND>]
 B<xdg-launch> {B<-X>|B<--xsession>}  [I<OPTIONS>] [I<ACTID>|I<DESKTOP>|I<COMMAND>]
 B<xdg-launch> {B<-E>|B<--startup>}   [I<OPTIONS>] [I<ACTID>|I<DESKTOP>|I<COMMAND>]
 B<xdg-launch> {B<-E>|B<--session>}   [I<OPTIONS>] [I<ACTID>|I<DESKTOP>|I<COMMAND>]
...
=item I<APPSPEC> = {I<ACTID>|I<MIMETYPE>|I<CATEGORY>|I<DESKTOP>}
...
=item I<ACTID> = I<APPID>[:I<ACTION>]

Specifies the application identifier and optional action of an XDG
desktop application to launch.
...
Specifies the action of an XDG desktop application to launch.  The
action must appear in the B<Actions> key in the desktop entry and have a
corresponding I<[Desktop Action ACTION]> group defining the action.
Note that auto-start desktop entries do no have actions.

The action, I<ACTION>, can alternately be specified as an option
argument using the B<--action> option.
...
=item [B<--desktop>] I<DESKTOP>

The XDG menu specification desktop name that identifies the X session
for which to launch.  This I<APPSPEC> is only applicable when
B<--xsession>, B<--startup> or B<--session> is specified.

The XDG menu specification desktop name, I<DESKTOP>, can alternatively
be specified as an option argument using the B<--desktop> option.  Where
the desktop name would conflict with an existing I<APPID>, it must be
specified with the B<--desktop> option.
...
the B<--file> option.  The option or argument can be repeated to specify
multiple files.
...
option.  The option or argument can be repeated to specify multiple
URLs.

=back

Mixing I<FILE> and I<URL> arguments or options is not currently
recommended.
...
key-value entry in the startup notification C<new:> message.
...
Specify an XDG menu specification desktop environment name, I<DESKTOP>,
that identifies the X session for which to launch.

=item B<-f>, B<--file> I<FILE>

Specifies a file name, I<FILE>, to be substituted into the XDG desktop
...
or no file name.  When B<--url> or URL is specified as a F<file://> URL,
...
Specifies that the launch should be chained from an preceding launch
operation.  This is the default when a B<DESKTOP_STARTUP_ID> environment
variable exists when B<xdg-launch> is initiated.  However, this option
can be used to ensure that an additional launch operation is not
initiated when B<xdg-launch> is used in the B<Exec> key of a desktop
entry file.  Also, when specified, none of the options that follow are
significant, with the exception of L</HISTORY OPTIONS> and L</GENERAL
OPTIONS>.  See L</LAUNCH CHAINING>, below, for detail.
...
When an autostart phase of C<Initialization> is specified, B<xdg-launch>
...
Delay for I<SECONDS> seconds before launching an auto-start entry.  The
default value is zero (0) seconds.  When, non-zero, this option
overrides the C<X-GNOME-AUtostart-Delay> desktop entry field.
...
=head2 LAUNCH CHAINING

So that the B<xdg-launch> command can be used within a desktop entry
file, B<xdg-launch> supports launch chaining.   When a
B<DESKTOP_STARTUP_ID> environment variable is set before the program
executed, B<xdg-launch> assumes that it has been executed from within a
desktop entry file.  The normal behaviour in this case is to act as
normal, except that a C<change:> message is sent where a C<new:> message
would be sent.

As, this may be confusing to some desktops or launch monitors, and may
confuse a window manager that supports desktop startup notification
completion, B<xdg-launch> attempts to detect the situation where it is
used to launch a desktop entry that uses B<xdg-launch> (or one of its
derivatives) in the B<Exec> line.  In the case that it detects a chained
invocation, it simply executes the command without setting the
B<DESKTOP_STARTUP_ID> and without sending a C<new:> message.  Then the
chained invocation will perform startup notification as though it was
invoked by itself.

To handle the case there a different launch is used to launch a desktop
entry that contains an B<xdg-launch> command, B<xdg-launch> also
supports the B<--chain> option.  When the B<--chain> option is
specified, the launch chaining behaviour is invoked.  The differences
are as follows:
...
When the B<xdg-launch> program starts, and there is no
B<DESKTOP_STARTUP_ID> environment variable set, it behaves normally;
when it is set, the chaining behaviour is invoked, and the following
differences apply:

=item 2.

When launching the chained application, no C<new:> or C<change:> message
will be sent and the B<DESKTOP_STARTUP_ID> environment variable will
be passed to the chained application unchanged.  The options described
under L</LAUNCH OPTIONS> and L</CONTEXT OPTIONS> are ignored.

=item 3.

No tool wait, assistance, wait for resources, nor content setting
procedures will be invoked.  The options described under L</ASSIST
OPTIONS>, L<TOOL WAIT OPTIONS>, L</SESSION OPTIONS> and the set
operations of L</CONTENT OPTIONS> are ignored.

=item 4.

The chained application will be executed in replacement for the
B<xdg-launch> program and no children will be spawned.

=back

Chaining launches in this fashion is intended to be used to allow an
B<xdg-launch> command in a desktop entry file.  Intended uses are with
the B<--mimetype> and B<--category> methods of specifying an application
to launch.  A set of "preferred" application desktop entries are
included in the distribution of B<xdg-launch>.  See the following
desktop entry files installed in the F</usr/share/applications>
directory.

=over

=item I<preffered-execute.desktop>

Launches the user's preferred run dialog with the B<Exec> key:
C<xdg-launch --chain RunCommand>.

=item I<preffered-file-manager.desktop>

Launches the user's preferred file manager with the B<Exec> key:
C<xdg-launch --chain FileManager>.

=item I<preffered-mail-reader.desktop>

Launches the user's preferred mail reader with the B<Exec> key:
C<xdg-launch --chain MailReader>.

=item I<preffered-terminal.desktop>

Launches the user's preferred terminal with the B<Exec> key:
C<xdg-launch --chain TerminalEmulator>.

=item I<preffered-text-editor.desktop>

Launches the user's preferred text editor with the B<Exec> key:
C<xdg-launch --chain TextEditor>.

=item I<preffered-web-browser.desktop>

Launches the user's preferred web browser with the B<Exec> key:
C<xdg-launch --chain text/html>.

=item I<preffered-calculator.desktop>

Launches the user's preferred calculator with the B<Exec> key:
C<xdg-launch --chain Calculator>.

=back

The B<xdg-launch> package installs a reasonable distribution-provided
F<prefapps.list> file alongside the F<mimeapps.list> file of the XDG
L<Association between MIME types and applications> specification.
...
See L</LAUNCH CHAINING> for details.
...
Used by B<xdg-launch> as the command to prefix to the exec line of
terminal applications to launch applications within a terminal.

=item B<XDG_TERMRESN>

Used by B<xdg-launch> as the command to prefix to the exec line of
terminal applications to launch applications within a terminal when a
resource name is available to be set on the terminal.  (Not necessarily
defined for all terminal types).

=item B<TERMINAL>

Use by B<xdg-launch> as the name of the preferred terminal program to
run.  If this terminal is known to B<xdg-launch>, it will automatically
determine the corresponding B<XDG_TERMINAL> and B<XDG_TERMRESN> commands
when they are not specified.  Terminal programs that are known to
B<xdg-launch> are: L<uxterm(1)>, L<xterm(1)>, L<st(1)>, L<pterm(1)>,
L<Eterm(1)>, L<aterm(1)>, L<urxvtc(1)>, L<urxvt(1)>, L<rxvt(1)>,
L<urxvt-tabbed(1)>, L<termit(1)>, L<terminator(1)>,
L<gnome-terminal(1)>, L<termite(1)>, L<mate-terminal(1)>,
L<terminology(1)>, L<kitty(1)>, L<alacritty(1)>, L<roxterm(1)>,
L<lxterminal(1)>, L<xfce4-terminal(1)>, L<lilyterm(1)>, L<tilix(1)>,
L<guake(1)>, L<x-terminal-emulator(1)>, L<deepin-terminal(1)>,
L<qterminal(1)>, L<konsole(1)>, L<tilda(1)> and L<koi8rxterm(1)>.

When no terminal environment variables are specified, terminal programs
will be detected in the user's B<PATH> in the order specified above.

Note that L<terminix(1)> and L<hyper(1)> are currently unknown.
...
=item F<$desktop-mimeapps.list>, F<mimeapps.list>

These files are searched in F<$XDG_CONFIG_HOME>, F<$XDG_CONFIG_DIRS>,
F<$XDG_DATA_HOME/applications>, F<$XDG_DATA_DIRS/applications> to
determine associations between I<MIMETYPE> and I<APPLICATION_ID>.  The
user's F<$XDG_CONFIG_HOME/$desktop-mimeapps.list> file is altered when
B<--set-default> or B<--set-preffered> are specified.

=item F<$desktop-prefapps.list>, F<prefapps.list>

These files are searched in F<$XDG_CONFIG_HOME>, F<$XDG_CONFIG_DIRS>,
F<$XDG_DATA_HOME/applications>, F<$XDG_DATA_DIRS/applications> to
determine associations between I<CATEGORY> and I<APPLICATION_ID>.  The
user's F<$XDG_CONFIG_HOME/$desktop-prefapps.list> file is altered when
B<--set-default> or B<--set-preffered> are specified.
