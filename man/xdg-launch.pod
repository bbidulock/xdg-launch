...
file, B<xdg-launch> support launch chaining.   When a
...
completion, B<xdg-launch> supports the B<--chain> option.  When the
B<--chain> option is specified, the launch chaining behaviour is
invoked.  The differences are as follows:

=over

=item 1.

When the B<xdg-launch> program starts, it fails when there is no
B<DESKTOP_STARTUP_ID> environment variable set.
...
be passed to the chained application.  The options described
under L</LAUNCH OPTIONS> and L</CONTEXT OPTIONS> are ignored.

=item 3.

No tool wait, assistance, wait for resources, not content setting
